...
var mobile = window.screen.height < 500
...
function mainPage(){
  ...
  let scale2 = vertical ? {s:3,w: c.w*.1, h: -c.h*.03} : mobile ? {s:1,w: c.w*.45, h: -c.h*.17} : {s:1,w: c.w*.45, h: -c.h*.1}
...
function map(){
  let scale1 = vertical ? {s:3,w: c.w*.1, h: 0} : mobile? {s:2,w: c.w*.2, h: -c.h*.16} : {s:2,w: c.w*.2, h: -c.h*.08}
  c.fillStyle = "lightblue"
...
function resize(){
  ...
  mobile = window.screen.height < 500
  c.w = innerWidth
  c.h = innerHeight
  buttons.length=0
